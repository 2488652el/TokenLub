 ...
 * 项目用量的徽标数量在运行时从 session-log 记录中统计并填充。
 * (glm-5.2)
 */
...
const NAV_SECTIONS: { items: NavItem[] }[] = [
  {
    items: [
      ...
      { to: '/agents', label: '项目用量', icon: 'fa-folder-tree' /* badge filled at runtime */ },
...
/**
 * 侧边导航栏组件。
 * 挂载时统计 session-log 中不同项目名称的数量,作为项目用量的徽标。
 */
export function Sidebar() {
  const [projectBadge, setProjectBadge] = useState<string | undefined>(undefined)
    ...
    window.api.usage
      ...
      .then((rows) => {
        ...
        for (const row of rows) {
          ...
          if (project) unique.add(project)
        }
        setProjectBadge(unique.size > 0 ? String(unique.size) : undefined)
      })
      .catch(() => {
  ...
  // 将运行时统计的项目数量注入到 /agents 导航项的徽标
  const sections = NAV_SECTIONS.map((s) => ({
    items: s.items.map((it): NavItem => {
      if (it.to !== '/agents') return it
      if (projectBadge === undefined) return it
      return { ...it, badge: projectBadge, badgeVariant: 'count' as const }
    })
  }))
