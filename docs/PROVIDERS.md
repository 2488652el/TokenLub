...
Providers not in this map are skipped because TokenLub cannot associate their usage with a supported provider. `newapi-generic` remains manual because each deployment can define its own multipliers and prices. To add a mapping, edit `PROVIDER_MAPPING` in `src/main/pricing/catalog.ts`.
