...
└──────────────────┬──────────────────────────────────────────┘
                   │ IPC (typed channels in src/shared/ipc-channels.ts)
┌──────────────────▼──────────────────────────────────────────┐
...
All names centralized in `src/shared/ipc-channels.ts` as `IPC.*`. Naming: `domain:verb` for request/reply; `subscribe:topic` for event push.

## Storage

- `better-sqlite3` database under `app.getPath('userData')`; migrations are implemented inline in `src/main/store/db.ts`.
- `app_settings` table for non-secret settings such as refresh interval.

## Provider plugin model

Every provider implements `ProviderImpl` from `src/shared/types/provider.ts`. New provider = drop a file under `src/main/providers/<id>/index.ts` and add to `registry.ts`.
