...
## macOS support (2026-07-13)

- [x] Centralized Windows/macOS Claude and Codex paths in `src/main/platform/paths.ts`.
- [x] Added renderer-safe `log:locations` IPC and removed `%USERPROFILE%` UI paths.
- [x] Centralized external-link allowlisting and preserved macOS window lifecycle behavior.
- [x] Made the better-sqlite3 Electron prebuild target platform/architecture aware.
- [x] Added macOS x64/arm64 DMG commands, ICNS icon, builder configuration, and packaged E2E coverage.
- [x] Windows regression gate: typecheck, 453 unit tests, build, and builder config check passed.
- [ ] macOS x64 and arm64: clean install, DMG build, mount, packaged E2E, and architecture checks.
- [ ] Formal release: Developer ID signing, notarization, staple, Gatekeeper, and SHA-256 recording.

Unsigned local DMGs are development artifacts only. Apple credentials must come from the
Keychain or environment variables and must never be committed or printed.
