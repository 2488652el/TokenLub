...
import { MAX_SYNC_V2_BYTES, SYNC_V2_PROTOCOL_VERSION, type SyncV2Snapshot } from '../shared/sync-v2'
...
async function route(
  ...
  if (ctx.request.method === 'GET' && ctx.url.pathname === '/console/tokenlub-mark.png') {
    return new Response(
      new Uint8Array(readFileSync(resolve('src/renderer/assets/tokenlub-mark.png'))),
      {
        headers: {
  ...
  if (ctx.request.method === 'GET' && ctx.url.pathname === '/console') {
    return new Response(readFileSync(resolve('src/server/console/index.html'), 'utf8'), {
      headers: {
