...
} from '../../../code/src/main/providers/endpoint-policy'
...
describe('provider endpoint policy', () => {
  it('requires HTTPS for hosted provider endpoints', () => {
    ...
    expect(validateProviderEndpoint('deepseek', 'https://attacker.example').ok).toBe(false)
  ...
  it('compares normalized origins', () => {
    expect(
      originChanged('deepseek', 'https://api.deepseek.com/v1', 'https://api.deepseek.com/v2')
    ).toBe(false)
    expect(originChanged('deepseek', 'https://api.deepseek.com', 'https://other.example')).toBe(
      true
    )
  })
})
