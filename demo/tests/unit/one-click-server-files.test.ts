...
it('ships a production sync stack with private PostgreSQL and HTTPS reverse proxy', () => {
  const compose = read('drive/docker-compose.server.yml')
  const sshCompose = read('drive/docker-compose.server-ssh.yml')
...
it('ships idempotent install and safe operational commands', () => {
  const install = read('drive/ops/one-click/install.sh')
  const backup = read('drive/ops/one-click/backup.sh')
  const upgrade = read('drive/ops/one-click/upgrade.sh')
  const healthcheck = read('drive/ops/one-click/healthcheck.sh')
  const uninstall = read('drive/ops/one-click/uninstall.sh')
