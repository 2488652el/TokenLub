...
const compose = readFileSync(resolve('drive/docker-compose.yml'), 'utf8')
const dockerfile = readFileSync(resolve('drive/Dockerfile'), 'utf8')
const deployGuide = readFileSync(resolve('drive/docs/ONE-CLICK-SERVER.md'), 'utf8')
const privacyAudit = readFileSync(resolve('drive/ops/privacy-audit.sh'), 'utf8')
const restoreRehearsal = readFileSync(resolve('drive/ops/restore-postgres-rehearsal.sh'), 'utf8')
const serverEntry = readFileSync(resolve('drive/src/server/index.ts'), 'utf8')
...
describe('container deployment contract', () => {
  ...
  it('provides a non-disclosing privacy audit for payloads and logs', () => {
    expect(deployGuide).toContain('sh drive/ops/privacy-audit.sh')
