...
vi.mock('../../../src/main/store/sync-session', () => ({
...
vi.mock('../../../src/main/sync/client', () => ({
  createSyncClient: vi.fn(() => state.client),
  loginSyncSession: state.loginSyncSession
}))
vi.mock('../../../src/main/sync/runner-v2', () => ({ runSyncV2Once: state.runSyncV2Once }))
vi.mock('../../../src/main/sync/scheduler', () => ({
...
describe('sync service', () => {
  ...
  it('initializes from encrypted session and exposes status without tokens', async () => {
    ...
    const service = await import('../../../src/main/sync/service')
