...
import { PostgresPhase1Store, type PostgresQueryClient } from '../../../src/server/postgres-store'
import type { SyncV2Snapshot } from '../../../src/shared/sync-v2'
...
describe('Sync V2 PostgreSQL migration', () => {
  it('defines one revisioned snapshot per user with JSON object validation', () => {
    const table = readFileSync(resolve('src/server/migrations/008_sync_v2_snapshot.sql'), 'utf8')
    const constraint = readFileSync(
      resolve('src/server/migrations/009_sync_v2_snapshot_shape.sql'),
      'utf8'
    )
