...
describe('discoverClaudeSessions', () => {
  ...
  it('recursively finds *.jsonl files', () => {
    ...
    mkdirSync(join(tmpRoot, 'proj1'), { recursive: true })
    mkdirSync(join(tmpRoot, 'proj1', 'subagents'), { recursive: true })
    writeFileSync(join(tmpRoot, 'proj1', 'a.jsonl'), '{"type":"assistant"}')
    writeFileSync(join(tmpRoot, 'proj1', 'subagents', 'b.jsonl'), '{"type":"assistant"}')
    writeFileSync(join(tmpRoot, 'proj1', 'readme.txt'), 'ignore me')
