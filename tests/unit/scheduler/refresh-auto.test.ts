...
import { getSetting, setSetting } from '../../../src/main/store/settings-store'
...
describe('scheduler auto-refresh timer', () => {
  ...
  afterEach(async () => {
    ...
    const { restartAutoRefresh } = await import('../../../src/main/scheduler/refresh')
    restartAutoRefresh()
    vi.useRealTimers()
    vi.clearAllMocks()
  })
  ...
  it('startAutoRefresh respects refresh_interval_min setting', async () => {
    ...
    const { startAutoRefresh } = await import('../../../src/main/scheduler/refresh')
  ...
  it('restartAutoRefresh stops old timer and starts new one with updated interval', async () => {
    ...
    const { startAutoRefresh, restartAutoRefresh } =
      await import('../../../src/main/scheduler/refresh')
