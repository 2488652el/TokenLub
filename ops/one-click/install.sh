...
prepare_source() {
  ...
  elif [[ -f "$SCRIPT_DIR/../../Dockerfile" && -f "$SCRIPT_DIR/../../docker-compose.server.yml" ]]; then
  ...
  [[ -f "$SOURCE_DIR/docker-compose.server.yml" ]] || die "源码目录缺少 docker-compose.server.yml：$SOURCE_DIR"
  [[ -f "$SOURCE_DIR/docker-compose.server-ssh.yml" ]] || die "源码目录缺少 docker-compose.server-ssh.yml：$SOURCE_DIR"
  [[ -f "$SOURCE_DIR/ops/one-click/Caddyfile.template" ]] || die '源码目录缺少 Caddyfile 模板。'
  if [[ "$SSH_ONLY" == 1 ]]; then
    COMPOSE_FILE="$SOURCE_DIR/docker-compose.server-ssh.yml"
  else
...
ensure_env() {
  ...
  if [[ "$SSH_ONLY" == 0 ]]; then
    cp "$SOURCE_DIR/ops/one-click/Caddyfile.template" "$INSTALL_DIR/Caddyfile"
...
exec bash "$SOURCE_DIR/ops/one-click/tokenlub-sync.sh" "\$@"
EOF
